/*if(collision_circle(x,y, 50, o_player, true, true) && room != r_title) {
	for (var i = 0; i < 1; i+=1) {
		var xx = x+lengthdir_x(12+i,-camera_get_view_angle(view_camera[0])+90)+lengthdir_x(36, image_angle);
		var yy = y+lengthdir_y(12+i,-camera_get_view_angle(view_camera[0])+90)+lengthdir_y(36, image_angle);

		if(o_hud.typer)
			draw_sprite_ext(s_mouse_interact, 0,xx,yy, .5, .5, image_angle, image_blend, image_alpha);
		else
			draw_sprite_ext(s_mouse_interact, 1,xx,yy, .5, .5, image_angle, image_blend, image_alpha);
		
		if(i == 0 && position_meeting(x+lengthdir_x(36, image_angle), y+lengthdir_y(36, image_angle), o_player)) {
			playerClose = true;
			if(!o_hud.typer)
				draw_sprite_ext(s_mouse_interact, 2,xx,yy, .5, .5, image_angle, image_blend, image_alpha);
			else
				draw_sprite_ext(s_mouse_interact, 3,xx,yy, .5, .5, image_angle, image_blend, image_alpha);
...
if(global.isCrateOpen && global.crate == self) {
	...
	var xx = o_hud.x+lengthdir_x(z, -global.camera_angle+90)+lengthdir_x(offX, angle)+lengthdir_x(offY, angle-90);
	var yy = o_hud.y+lengthdir_y(z, -global.camera_angle+90)+lengthdir_y(offX, angle)+lengthdir_y(offY, angle-90);
	...
	if(takeAllSlot.highlighted)
		draw_sprite_ext(s_icons, 1,xx,yy, image_xscale/1.25, image_yscale/1.25, angle, image_blend, 1);
	else
		draw_sprite_ext(s_icons, 1,xx,yy, image_xscale/1.5, image_yscale/1.5, angle, image_blend, 1);
	...
	for(var ix = 0; ix < width; ix++)
		for(var iy = 0; iy < height; iy++) {
			...
			if(quickMove && quickMoveX == ix && quickMoveY == iy)
				draw_sprite_ext(s_quickmove, quickMoveLoad+11,xx,yy, image_xscale/1.75, image_yscale/1.75, angle, image_blend, 1);
