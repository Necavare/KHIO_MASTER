...
if(!global.isInventoryOpen && !global.isPaused && !global.minimapFullscreen && position_meeting(mouse_x, mouse_y, self) && collision_circle(o_player.x, o_player.y, 30, self, true, false) && !global.isCrateOpen && playerNear && !isOnFire && !global.isInventoryOpen && (global.interactingObject == -1 || global.interactingObject == self)) {
...
if(options && (!mouse_check_button(global.M_LEFT) || !collision_circle(o_player.x, o_player.y, 30, self, true, false)) && !isOnFire) {
	...
	if(window_mouse_get_x() > mouseX + 10) {
		...
		audio_play_sound_on(crate, sn_smallcrate_open, 0, 1);
	} 
}
