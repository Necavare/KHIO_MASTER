...
#region Movement
	...
	if(isStunned){
		...
		if((playerInRange && !isDying) || ( !isDying && isAttacking)){
			...
			if(!hasDodged && (hasParried || hasBlocked) && !hasSwung && img_frame == 0){
				show_debug_message("going into the if");
			...
			else if(hasDodged && (hasParried || hasBlocked)){
				if(heavyAttackChance && !hasSwung && playerBlocking && img_frame == 0){
					show_debug_message("going into the if2");
					playerBlockingBlue = false;
					isRunning = false;
					isWalking = false;
					isIdle = true;
...
if(o_player.isHoldingTool && place_meeting(x,y,o_tool) && o_tool.isSwung && canHit) {
	...
	if(!(isAttacking && img_frame <= 3) && !isStunned){
