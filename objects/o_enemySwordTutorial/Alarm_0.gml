...
if(isIdle && !playerInRange && !isAttacking) { // Idle framerate
	if(!isStunned && !isStumbled){
		...
		if(parriedCur && !playerAction){
			show_debug_message("parried cur");
			
			room_speed = 20;
			global.tutorialAttack = true;
		}
		if(img_frame < 12)
			img_frame++;
		else{
	...
	if(delayAttack){
		...
		if(heavyAttack){
			...
			if(o_player.isRolling && !hasDodged){
				rolledCur = true;
				hasDodged = true;
			}
			/*
			...
			else if(!hasDodged && !playerAction){
				global.tutorialDodge = true;
				room_speed = 20;	
			}
			...
			show_debug_message("hasBlocked: "+string(hasBlocked));
			if(parriedCur && !playerAction){
				room_speed = 20;
				global.tutorialAttack = true;
			}
			else if(!hasParried && !hasBlocked && !playerAction){
				global.tutorialParry = true;
				room_speed = 20;	
			}
			...
			if(img_frame < 	9)
				...
				/*
				if(hasDodged && (hasParried || hasBlocked)){
					if(irandom(99)<30){
						heavyAttack = true;	
					}
				}*/
